...
static sgx_status_t SGX_CDECL sgx_generate_hasharray(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_generate_hasharray_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_generate_hasharray_t* ms = SGX_CAST(ms_generate_hasharray_t*, pms);
	sgx_status_t status = SGX_SUCCESS;



	ms->ms_retval = generate_hasharray(ms->ms_numhashes);


	return status;
}

static sgx_status_t SGX_CDECL sgx_doASLR(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_doASLR_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_doASLR_t* ms = SGX_CAST(ms_doASLR_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
...
SGX_EXTERNC const struct {
	...
	3,
	{
		...
		{(void*)(uintptr_t)sgx_generate_hasharray, 0, 0},
		{(void*)(uintptr_t)sgx_doASLR, 0, 0},
	}
};
